...
public class LocatedInBuilder implements Reloadable
{
  ...
  private void buildFunction()
  {
    ...
    String sql = "" +
        ...
        + "BEGIN\n"
        + "  BEGIN\n"
        + "    FOR parentMd IN SELECT "+PARENT_CLASS+" FROM "+UNIVERSALS_TABLE+" WHERE "+CHILD_CLASS+" = currentMd LOOP\n"
        + "      foundMatch := false;\n"
        ...
        + "        AND CASE WHEN st_dimension(child.text_geom) = 1 THEN st_crosses(st_geomfromtext(child.geo_data, st_srid(parents.geom)), parents.geom)\n"
        ...
        + "    INSERT INTO "+FAILED_ENTITIES_TABLE+" ("+id+") VALUES (childId);"
        + "    RETURN;\n" 
        + "  END;\n" 
        + "END\n" 
        + "$$\n"
        ...
        + "        RETURN QUERY SELECT * FROM "+DERIVE_LOCATED_IN_REC_FUNC+"(pctThreshold, childRow."+id+", childRow.md);\n"
        ...
        + "        RETURN QUERY SELECT * FROM "+DERIVE_LOCATED_IN_FUNC+"_rec(pctThreshold, childRow."+id+", childRow.md);\n"
        + "        perform nextval('"+GEO_PROGRESS_SEQ+"');\n"
        + "      END LOOP;\n" 
        + "    ELSE\n" 
        + "      RETURN;\n" 
...
//    vacuumGeom();
  ...
  public OIterator<ValueObject> deriveLocatedIn()
  {
    ...
    vq
        .FROM(""+DERIVE_LOCATED_IN_FUNC+"(" + percent + "::integer, " + type.getCode() + "::integer)",
            "derivation");
    
    ValueQuery liVQ = new ValueQuery(f);
    LocatedInQuery liQ = new LocatedInQuery(liVQ);
    
    liVQ.SELECT(liVQ.aSQLCharacter("li_child_id", liQ.childId().getDbQualifiedName()), 
        liVQ.aSQLCharacter("li_parent_id", liQ.parentId().getDbQualifiedName()));
