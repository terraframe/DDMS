...
import dss.vector.solutions.gis.MainWindow;
...
public class LocatedInWizard extends Wizard implements Reloadable
{
  ...
  {
    setWindowTitle(GISAdminLocalizer.getMessage("LOCATED_IN_WIZARD"));
  ...
  public boolean performFinish()
  {
    try
    {
      File file = new File(MainWindow.LOG_DIR + File.separator + LOCATED_IN_LOG_DIR + File.separator + System.currentTimeMillis() + MainWindow.LOG_EXT);
      ...
      try
      {
        ...
        if (logger.hasLogged())
        {
          String message = GISAdminLocalizer.getMessage("ERROR_IN_LOCATED_IN") + " " + file.getAbsolutePath();

          MessageDialog.openConfirm(getShell(), GISAdminLocalizer.getMessage("MESSAGE"), message);
        }
      }
      finally
      {
  ...
  private void handleException(Throwable throwable)
  {
    ...
    {
      String message = GISAdminLocalizer.getMessage("UNKNOWN_LOCATED_IN_EXCEPTION");
  ...
  public boolean performCancel()
  {
    if (!this.isImporting())
    {
      return MessageDialog.openConfirm(getShell(), GISAdminLocalizer.getMessage("CONFIRMATION"), GISAdminLocalizer.getMessage("CANCEL_MESSAGE"));
    }
