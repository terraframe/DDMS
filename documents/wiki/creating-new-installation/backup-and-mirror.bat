...
REM ##### Variables #########
SET DDMS_DIR=C:\Users\Administrator\DDMS
SET BACKUP_DIR=%DDMS_DIR%\backups
SET BACKUP_LOG=%DDMS_DIR%\logs\backup.log
SET TRANSFER_DIR=%DDMS_DIR%\transfer
REM ##### Variables #########

powershell -Command "powershell -ExecutionPolicy ByPass -NoLogo -NonInteractive -NoProfile -WindowStyle Hidden -File C:\MDSS\manager\manager.ps1 -backupAll %BACKUP_DIR% > %BACKUP_LOG%"
