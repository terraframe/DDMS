...
<div id="app-container" class="container" >
  ...
  <div>
    <span>
    	<h2> <mdss:localize key="category.icon.title"/> </h2>
    </span>
  </div>
  ...
  <div class="list-table-wrapper">
      ...
      <thead>
        <tr>
          <th></th>
          <th class="label-column"><mdss:localize key='category.icon.label'/></th>
          <th class="label-column"><mdss:localize key='category.icon.preview'/></th>
        </tr>
      </thead>	  
	    <tbody>
	      <tr ng-repeat="icon in icons" class="fade-ngRepeat-item">
	        <td class="button-column">
	          <span>
	            <a class="fa fa-pencil ico-edit" ng-click="ctrl.edit(icon)" title="<mdss:localize key="category.icon.editTooltip"/>"></a>                     
	            <a class="fa fa-trash-o ico-remove" ng-click="ctrl.remove(icon)" title="<mdss:localize key="category.icon.removeTooltip"/>"></a>           
	          </span>
	        </td>
	...
	<!--      REMOVED:  onerror="if (this.src != 'net/geoprism/images/dashboard_icon_small.png') this.src = 'net/geoprism/images/dashboard_icon_small.png';" -->
        ...
        <tr>
          <td class="button-column">
            <a class="fa fa-plus" ng-click="ctrl.add()" title="<mdss:localize key="category.icon.addTooltip"/>"></a>
          </td>                 
          <td colspan="2">
          </td>                 
        </tr>	      
  ...
  <div ng-if="icons === null"><mdss:localize key='dataset.loadingData'/></div>
  <div ng-if="icons.length === 0">
    <p><mdss:localize key='category.icon.emtpy'/></p>
  </div>
		  ...
		  <form class="modal-form" name="ctrl.form">
		    <div class="modal-dialog" ng-if="show">
		      <div class="modal-content" show-on-ready>
		             <div class="heading">
		                <h1 ng-if="editIcon"><mdss:localize key="category.icon.editHeader"/></h1>
		                <h1 ng-if="!editIcon"><mdss:localize key="category.icon.addHeader"/></h1>
		             </div>      
		        <fieldset>
		          ...
		          <div class="row-holder">
		            <div class="label-holder">
		              <label><mdss:localize key="category.icon.label"/></label>
		            </div>          
		            <div class="holder" >
		              <span class="text">
		                  ...
		                  <div class="drop-box">
		                    <div class="inner-drop-box">
		                      <i class="fa fa-cloud-upload">
		                        <p class="upload-text"><mdss:localize key="category.icon.uploadIcon"/></p>
		                      </i>
		                    </div>
		                  </div>
		                </div>
		                <div ng-show="icon.file">
		                  <a style="font-size:25px;vertical-align:middle;" class="fa fa-trash-o ico-remove" ng-click="icon.file = null" title="<mdss:localize key="category.icon.removeFile"/>"></a>           
		                  ...
		                  <span ng-show="ctrl.form.$error.file" style="float: right;">
		                    <p class="error-message"><mdss:localize key="category.icon.badFileType"/></p>
		                  </span>
		                </div>          
		              </span>
		            </div>
		          ...
		          <div class="row-holder" fire-on-ready>
		            ...
		            <div class="holder">
		              <div class="button-holder">
		                <input type="button" value="<mdss:localize key="dataset.cancel"/>" class="btn btn-default" ng-click="ctrl.cancel()" />              
		                <input ng-show="!editIcon" type="button" value="<mdss:localize key="category.icon.ok"/>" class="btn btn-primary" ng-click="ctrl.create()" ng-disabled="ctrl.form.$invalid" />                
		                <input ng-show="editIcon" type="button" value="<mdss:localize key="category.icon.ok"/>" class="btn btn-primary" ng-click="ctrl.apply()" ng-disabled="ctrl.form.$invalid" />
		              </div>
		            </div>
		          </div>
		        </fieldset>  
