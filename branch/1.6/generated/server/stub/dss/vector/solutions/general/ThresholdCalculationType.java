...
public class ThresholdCalculationType extends ThresholdCalculationTypeBase implements com.runwaysdk.generation.loader.Reloadable
{
  ...
  public void apply()
  {
    if (this.isDifferent())
    {
      ...
      this.validateWeeksBefore();
      this.validateWeeksAfter();
  ...
  {
    if (this.getWeeksAfter() != null && this.getWeeksAfter() > 4)
    {
      RangeValueProblem p = new RangeValueProblem();
      p.setNotification(this, WEEKSAFTER);
      p.setAttributeDisplayLabel(getWeeksAfterMd().getDisplayLabel(Session.getCurrentLocale()));
      p.setInvalidValue(this.getWeeksAfter());
      p.setLowerLimit(0);
      p.setUpperLimit(4);
      p.apply();
  ...
  {
    if (this.getWeeksBefore() != null && this.getWeeksBefore() > 4)
    {
      RangeValueProblem p = new RangeValueProblem();
      p.setNotification(this, WEEKSBEFORE);
      p.setAttributeDisplayLabel(getWeeksBeforeMd().getDisplayLabel(Session.getCurrentLocale()));
      p.setInvalidValue(this.getWeeksBefore());
      p.setLowerLimit(0);
      p.setUpperLimit(4);
      p.apply();
