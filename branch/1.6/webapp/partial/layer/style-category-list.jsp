...
<div class="panel-group choice-color category-group">
          ...
          <div class="category-container">
            <div class="text category-input-container">
              <div class="category-input-sub-container" ng-class="{'left' : (categories.rangeCategoriesEnabled)}">
              	<i class="range-cat-ico range-cat-left-ico" ng-class="{'active' : (category.rangeAllMin)}" ng-click="toggleMin($event)" ng-show="categories.rangeCategoriesEnabled" title="<mdss:localize key="DashboardThematicLayer.form.minCatRangeTooltip"/>">&#60;</i>
              	<input class="category-input left" type="text" ng-class="{'category-range-input' : (categories.rangeCategoriesEnabled), 'active' : (category.rangeAllMin)}" ng-disabled="category.rangeAllMin" ng-model="category.val" ng-required="categoryCheck()" placeholder="<mdss:localize key="DashboardLayer.form.catPlaceHolder"/>" autocomplete="on" category-auto-complete source="autoComplete()" number-only enforce="{{type === 'number'}}"></input>
              </div>
              <div class="category-input-sub-container right">
              	<i class="range-cat-ico range-cat-right-ico" ng-class="{'active' : (category.rangeAllMax)}" ng-click="toggleMax($event)" ng-show="categories.rangeCategoriesEnabled" title="<mdss:localize key="DashboardThematicLayer.form.maxCatRangeTooltip"/>">&#62;</i>
              	<input class="category-input category-range-input cat-range-max-val right" ng-class="{'active' : (category.rangeAllMin)}" ng-disabled="category.rangeAllMax" type="text" ng-model="category.valMax" ng-required="categoryCheck()" ng-show="(categories.rangeCategoriesEnabled)" placeholder="<mdss:localize key="DashboardLayer.form.catPlaceHolder"/>" autocomplete="on" category-auto-complete source="autoComplete()" number-only enforce="{{type === 'number'}}"></input>
              </div>	
            </div>
            <div class="cell">
  ...
  <div class="style-options-block" ng-if="type === 'number' && is-secondary-attribute-list === 'false'">    
    <styled-check-box id="cat-range-categories" model="categories.rangeCategoriesEnabled" label="<mdss:localize key="DashboardThematicLayer.form.categoryRangeCategoriesLabel"/>"></styled-check-box>
  </div>
</div>
