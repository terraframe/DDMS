...
Function upgradePostgresAndPostgis
  ...
  File postgis_restore.exe
  File postgis_restore_launcher.bat
...
Function migrateDatabaseSoftware
  ...
  LogEx::Write "Installing Postgres 9.6."
	SetOutPath $INSTDIR
  ${If} ${RunningX64}
	  File ../thirdparty/postgres/postgresql-9.6.3-2-windows-x64.exe
	  ...
	  Call execDos
	${Else}
	  File ../thirdparty/postgres/postgresql-9.6.3-2-windows.exe
  ...
  ${If} $0 == 5
    File "/oname=$INSTDIR\${POSTGRES_DIR}\data\pg_hba.conf" "..\installer\pg_hba_ipv4.conf"

  # Version 5 means Vista or Seven.  IPv6 enabled
  ${ElseIf} $0 == 6
    File "/oname=$INSTDIR\${POSTGRES_DIR}\data\pg_hba.conf" "..\installer\pg_hba_ipv6.conf"
  ...
  File "/oname=$INSTDIR\${POSTGRES_DIR}\data\postgresql.conf" "..\installer\postgresql.conf"
	...
	${If} ${RunningX64}
	  ...
	  File /r "../thirdparty/postgis/postgis-bundle-pg96-2.3.2x64/*"
	  push `"$INSTDIR\migrate\postgis\makepostgisdb_using_extensions.bat"`
	  Call execDos
	${Else}
	  SetOutPath "$INSTDIR\migrate\postgis"
	  File /r "../thirdparty/postgis/postgis-bundle-pg96-2.3.1x32/*"
	  push `"$INSTDIR\migrate\postgis\makepostgisdb_using_extensions.bat"`
	  Call execDos
	${EndIf}
