...
CREATE OR REPLACE FUNCTION postgis_scripts_build_date() RETURNS text
	AS 'SELECT ''2016-11-29 00:39:03''::text AS version'
	LANGUAGE 'sql' IMMUTABLE;
...
END
$postgis_proc_upgrade$;
-- Operator family brin_geometry_inclusion_ops_2d brin -- LastUpdated: 203
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
BEGIN
  IF 203 > version_from_num FROM _postgis_upgrade_info THEN
    EXECUTE $postgis_proc_upgrade_parsed_def$ CREATE OPERATOR FAMILY brin_geometry_inclusion_ops_2d USING brin;
...
END
$postgis_proc_upgrade$;
-- Operator family brin_geometry_inclusion_ops_3d brin -- LastUpdated: 203
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
BEGIN
  IF 203 > version_from_num FROM _postgis_upgrade_info THEN
    EXECUTE $postgis_proc_upgrade_parsed_def$ CREATE OPERATOR FAMILY brin_geometry_inclusion_ops_3d USING brin;
...
END
$postgis_proc_upgrade$;
-- Operator family brin_geometry_inclusion_ops_4d brin -- LastUpdated: 203
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
BEGIN
  IF 203 > version_from_num FROM _postgis_upgrade_info THEN
    EXECUTE $postgis_proc_upgrade_parsed_def$ CREATE OPERATOR FAMILY brin_geometry_inclusion_ops_4d USING brin;
...
CREATE OR REPLACE FUNCTION _add_raster_constraint_extent(rastschema name, rasttable name, rastcolumn name)
	...
	BEGIN
		...
		sql := 'SELECT MAX(@extschema@.ST_SRID('
            || quote_ident($3)
      || ') ) FROM '
            || fqtn
            || ' HAVING COUNT(DISTINCT @extschema@.ST_SRID('
            || quote_ident($3)
      || ') ) = 1';
