package dss.vector.solutions.geo.generated;

@com.runwaysdk.business.ClassSignature(hash = -773212693)
/**
 * This class is generated automatically.
 * DO NOT MAKE CHANGES TO IT - THEY WILL BE OVERWRITTEN
 * Custom business logic should be added to SettlementSubdivision.java
 *
 * @author Autogenerated by RunwaySDK
 */
public abstract class SettlementSubdivisionBase extends dss.vector.solutions.geo.generated.GeoEntity implements com.runwaysdk.generation.loader.Reloadable
{
  public final static String CLASS = "dss.vector.solutions.geo.generated.SettlementSubdivision";
  private static final long serialVersionUID = -773212693;
  
  public SettlementSubdivisionBase()
  {
    super();
  }
  
  protected String getDeclaredType()
  {
    return CLASS;
  }
  
  public static SettlementSubdivisionQuery getAllInstances(String sortAttribute, Boolean ascending, Integer pageSize, Integer pageNumber)
  {
    SettlementSubdivisionQuery query = new SettlementSubdivisionQuery(new com.runwaysdk.query.QueryFactory());
    com.runwaysdk.business.Entity.getAllInstances(query, sortAttribute, ascending, pageSize, pageNumber);
    return query;
  }
  
  public static SettlementSubdivision get(String id)
  {
    return (SettlementSubdivision) com.runwaysdk.business.Business.get(id);
  }
  
  public static SettlementSubdivision getByKey(String key)
  {
    return (SettlementSubdivision) com.runwaysdk.business.Business.get(CLASS, key);
  }
  
  public static SettlementSubdivision lock(java.lang.String id)
  {
    SettlementSubdivision _instance = SettlementSubdivision.get(id);
    _instance.lock();
    
    return _instance;
  }
  
  public static SettlementSubdivision unlock(java.lang.String id)
  {
    SettlementSubdivision _instance = SettlementSubdivision.get(id);
    _instance.unlock();
    
    return _instance;
  }
  
  public String toString()
  {
    if (this.isNew())
    {
      return "New: "+ this.getClassDisplayLabel();
    }
    else
    {
      return super.toString();
    }
  }
}
