...
public class TransactionItemView extends TransactionItemViewBase implements Reloadable
{
  ...
  public static TransactionRecordQuery getRecordQuery(String sortAttribute, Boolean isAscending, Integer pageSize, Integer pageNumber)
  {
    ...
    {
      if (sortAttribute.equals(TransactionRecord.CREATEDATE)) query.ORDER_BY(query.getCreateDate(), sortOrder);
      if (sortAttribute.equals(TransactionRecord.SEQ)) query.ORDER_BY(query.getSeq(), sortOrder);
      if (sortAttribute.equals(TransactionRecord.EXPORTSEQUENCE)) query.ORDER_BY(query.getExportSequence(), sortOrder);
      if (sortAttribute.equals(TransactionRecord.SITEMASTER)) query.ORDER_BY(query.getSiteMaster(), sortOrder);
    }
