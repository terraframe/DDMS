...
import com.terraframe.mojo.dataaccess.attributes.value.AttributeInteger;
...
*/
	...
	private double[] calculateWeightedSeasonalMeans(GeoEntity geoEntity, int week, int year) {
		...
		for (int i = 0; i < this.getPriorYears(); i++) {
			...
			long totalCases = 0;
			if (this.getCaseTypes().contains(ThresholdCalculationCaseTypes.INDIVIDUAL) || this.getCaseTypes().contains(ThresholdCalculationCaseTypes.BOTH)) {
				totalCases += this.getIndividualCount(factory, entityQuery, initialWeek, finalWeek);
			}
			if (this.getCaseTypes().contains(ThresholdCalculationCaseTypes.AGGREGATED) || this.getCaseTypes().contains(ThresholdCalculationCaseTypes.BOTH)) {
				totalCases += this.getAggregatedCount(factory, entityQuery, initialWeek, finalWeek);
			}
			double seasonalMean = (double) totalCases / (double) (this.getWeeksBefore() + this.getWeeksAfter() + 1);
	...
	@Transaction
	private long getAggregatedCount(QueryFactory factory, GeoEntityQuery entityQuery, EpiDate initialWeek, EpiDate finalWeek) {
