    ...
    Instance : {
      ...
      getData : function(callback) {
        if (this._isInitialized === false) {
          throw new com.runwaysdk.Exception(com.runwaysdk.Localize("rInitalSetup", "The data source must perform initial setup first. This is accomplished by invoking 'initialSetup' on the data source before invoking any other methods."));
        }
      ...
      handleException : function(ex, throwIt) {
        var dialog = com.runwaysdk.ui.Manager.getFactory().newDialog(com.runwaysdk.Localize("rError", "Error"), {modal: true});
        dialog.appendContent(ex.getLocalizedMessage() || ex.getMessage() || ex.getDeveloperMessage());
        dialog.addButton(com.runwaysdk.Localize("rOk", "Ok"), function(){dialog.close();});
        dialog.render();
