...
Mojo.Meta.newClass('MDSS.GeoHierarchyTree', {
  Instance : {
    ...
    /**
     * Creates an HTML node from the the given TermView. If pureObject is set to
     * true, then the returned node will be an object literal as opposed to a
     * HTMLNode.
     */
    _createNode : function(view, parent)
    {
      ...
      var data = {
        geoHierarchyId : view.getGeoHierarchyId(),
        geoHierarchyView : view
      };

      return new YAHOO.widget.HTMLNode({
        html : html,
        data : data
      }, parent);
    },
      ...
      var request = new MDSS.Request({
        onSuccess : function(geoHierarchyId)
        {
          ...
          var request = new MDSS.Request({
            ...
            {
              // add the node directly if the children have already been
              // dynamically loaded
              if (that._selectedNode.dynamicLoadComplete)
              {
                ...
                {
                  // don't expand the node if the parent's children haven't been
                  // loaded (it's wasteful)
                  if (parent.dynamicLoadComplete)
                  {
                    ...
                    // if there is only one child and it is already expanded. So
                    // force
    ...
    /**
     * Performs the DOM level cleanup after a hierarchy has been deleted.
     * 
     * @param deleteAll
     *          denotes if the deleted entity should be removed from all parent
     *          nodes (as opposed to the current parent).
     * 
     * @param ids
     *          array of ids whose geo hierarchy nodes need to be moved under
     *          Earth
     * 
     * @param geoHierarchyId
     *          The id of the geohierarchy to be deleted.
     */
    _postDeleteCleanup : function(deleteAll, ids, geoHierarchyId)
    {
      if (deleteAll)
      {
        function deleteNodeFromTree(id, earth)
        {
          ...
          Mojo.Iter.forEach(nodes, function(node)
          {
            // simplify things by moving the first node under earth. Anything
    ...
    /**
     * Callback for when a user has chosen to delete a GeoHierarchy and had to
     * confirm whether to delete the object itself or its relationship with the
     * current parent.
     */
    _deleteAfterConfirmation : function(e, obj)
    {
      ...
      var request = new MDSS.Request({
        ...
        onSuccess : function(ids)
        {
          ...
          that._postDeleteCleanup(this.deleteAll, ids, this.geoHierarchyId);
        }
      });
    ...
    _performDelete : function(destroyModal, geoHierarchyView)
    {
      ...
      var request = new MDSS.Request({
        ...
        onConfirmDeleteHierarchyException : function(e)
        {
          ...
          YAHOO.util.Event.on(delRel, 'click', Mojo.Util.bind(that, that._deleteAfterConfirmation), delRelObj);
    ...
    _dragDropHandler : function(source, id)
    {
      ...
      var request = new MDSS.Request({
        ...
        onConfirmHierarchyParentChangeException : function(e)
        {
          ...
          YAHOO.util.Event.on(yes, 'click', Mojo.Util.bind(that, that._addChildToParent), {
            clone : false,
          ...
          YAHOO.util.Event.on(no, 'click', Mojo.Util.bind(that, that._addChildToParent), {
            clone : true,
            references : this.references
          }); // this == tree
    ...
    _setupTree : function(geoHierarchyView)
    {
      var node = {
        type : "HTML",
        html : geoHierarchyView.getDisplayLabel()
      };
