...
public class ClassifierCondition extends DashboardPrimitiveCondition implements Reloadable
{
  ...
  public void restrictQuery(ValueQuery vQuery, Selectable attribute)
  {
    ...
    if (mdBusinessDAO.definesType().equals(Term.CLASS))
    {
      try
      {
        AllPathsQuery allPathQuery = new AllPathsQuery(vQuery);
        ...
        for (int i = 0; i < array.length(); i++)
        {
          ...
          String termId = object.getString("id");
  ...
  public List<String> getConditionInformation()
  {
    ...
    try
    {
      ...
      for (int i = 0; i < array.length(); i++)
      {
        ...
        try
        {
          Term classifier = Term.get(termId);

          String localizedValue = classifier.getTermDisplayLabel().getValue();
